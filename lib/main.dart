...
class ShoppingCartPage extends StatelessWidget {
  ...
  ShoppingCartPage(
      {required this.cart, required this.vegetableList, required this.fruitList});

  void resetCartAndItems() {
    cart.items.clear();
    vegetableList.forEach((item) => item.selected = false);
    fruitList.forEach((item) => item.selected = false);
  }
        ...
        child: Column(
          ...
          children: [
            ...
            for (var item in widget.itemList)
              ListTile(
                title: Text('${item.name} - \$${item.price} per kg',
...
class CheckoutPage extends StatelessWidget {
  ...
  CheckoutPage({required this.cart, required this.onCancel, required this.onSubmit});

  @override
  Widget build(BuildContext context) {
    double total =
    cart.items.map((item) => item.price).fold(0, (a, b) => a + b);
        ...
        child: Column(
          ...
          children: [
            ...
            for (var item in cart.items)
              Text('${item.name} - \$${item.price} per kg',
                  style: TextStyle(fontSize: 16)),
            SizedBox(height: 20),
